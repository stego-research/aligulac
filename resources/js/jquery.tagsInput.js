(function($) {
	...
	$.fn.doAutosize = function(o){
	    ...
	    var testerWidth = testSubject.width(),
	        ...
	        isValidWidthStr = (newWidth < currentWidth && newWidth >= minWidth)
    ...
    var settings = jQuery.extend({
      ...
      formatAutocomplete: function (item) { return item.value; }
		...
		this.each(function() { 
			...
			var $real_input = $(this);
			var $holder = $(markup).insertAfter($real_input);
			var $fake_input = $holder.find('#'+id+'_tag');
				...
				if (settings.autocomplete_url != undefined) {
					...
					if (jQuery.Autocompleter !== undefined) {
						...
						$fake_input.bind('result',data,function(event,data,formatted) {
							if (data) {
								$real_input.addTag(data[0] + "",{focus:true,unique:(settings.unique)});
							}
					  	});
					} else if (jQuery.ui.autocomplete !== undefined) {
					    $fake_input.autocomplete(autocomplete_options).data("ui-autocomplete")._renderItem = function (ul, item) {
					        return $("<li></li>")
								.append(settings.formatAutocomplete(item))
								.appendTo(ul);
					    };
						$fake_input.bind('autocompleteselect',data,function(event,ui) {
							$real_input.addTag(ui.item.value,{focus:true,unique:(settings.unique)});
							return false;
						});
						if(autocomplete_options.minLength === 0){
							$fake_input.focus(function(){ $(this).autocomplete('search');});
						}
					}
						...
						$fake_input.bind('blur',data,function(event) { 
							...
							if ($this.val()!='' && $this.val()!=d) { 
								if( (event.data.minChars <= $this.val().length) && (!event.data.maxChars || (event.data.maxChars >= $this.val().length)) )
									$real_input.addTag($this.val(),{focus:true,unique:(settings.unique)});
							} else {
