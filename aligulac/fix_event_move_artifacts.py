...
bad_events = Event.objects.raw('''
                               ...
                               FROM event,
                                    eventadjacency
                               WHERE event.parent_id != eventadjacency.parent_id AND 
...
cur.execute('''
            UPDATE event
            SET parent_id = eventadjacency.parent_id FROM eventadjacency
            WHERE
                event.parent_id != eventadjacency.parent_id
              AND
                eventadjacency.child_id = event.id
              AND
                distance = 1;''')
